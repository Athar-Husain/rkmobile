...
const sendFCMTokenToServer = async (authToken) => {
    try {
        const fcmToken = await getFCMToken()
        if (!fcmToken) {
            return
...
const AuthService = {
    ...
    signupVerifyOTP1: async (data) => {
        ...
        if (token && expiresIn) {
            await TokenManager.save(token, expiresIn)
            // Send FCM token to server in the background
    ...
    signupVerifyOTP: async (data) => {
        const fcmToken = await messaging().getToken()
        const res = await axiosInstance.post(`${AUTH_URL}/signup/verify-otp`, {
            ...data,
    ...
    signinVerifyOTP: async (data) => {
        const fcmToken = await messaging().getToken()
        const res = await axios.post(`${AUTH_URL}/signin/verify-otp`, {
            ...data,
            deviceToken: fcmToken,
            platform: Platform.OS === 'ios' ? 'ios' : 'android',
        })
    ...
    logout: async () => {
        ...
        try {
            ...
            console.error('Error getting FCM token on logout:', error)
        }

        // Call the logout endpoint with the device token
        await axiosInstance.post('/logout', { deviceToken })
