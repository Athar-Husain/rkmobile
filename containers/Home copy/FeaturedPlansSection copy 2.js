...
const PlanCard = ({ item, dark }) => {
    ...
    const internetSpeed = item?.internetSpeed || 'N/A'
    const internetSpeedUnit = (item?.internetSpeedUnit || '').toUpperCase()
    const description = item?.description || 'No description available.'
    const price = item?.price !== undefined ? `₹${item.price}` : '$N/A'
    const duration = (item?.duration || '').replace('_', ' ') || 'N/A'
    ...
    return (
        <TouchableOpacity
            ...
            accessibilityLabel={`${planName}, ${internetSpeed} ${internetSpeedUnit}, ${price} per ${duration}`}
        >
            <LinearGradient
...
FeaturedPlansSection.propTypes = {
    data: PropTypes.arrayOf(
        PropTypes.shape({
            _id: PropTypes.oneOfType([PropTypes.string, PropTypes.number]), // Make it not required or handle undefined
            name: PropTypes.string,
            internetSpeed: PropTypes.number,
            internetSpeedUnit: PropTypes.string,
