...
const SupportTicketDetail = () => {
    ...
    // console.log('tickeId from params', ticketId)
    ...
    useEffect(() => {
        if (flatListRef.current && publicComments.length > 0) {
    ...
    const handleSendComment = async () => {
        ...
        try {
            ...
            setMessage('')
            setMedia(null)
            // The onContentSizeChange prop will handle the scroll after the new comment is added.
        } catch (err) {
    ...
    //             ticketId,
    //             content: message.trim(),
    //             media, // Attach any media if applicable
    //         }

    //         // Emit the comment via socket to server
    //         socket.emit('addPublicComment', commentObj)

    //         // Immediately update Redux store without waiting for backend response
    //         dispatch(addRealTimePublicComment(commentObj))

    //         // Reset the message and media
    //         setMessage('')
    //         setMedia(null)
    //     } catch (err) {
    //         console.error('Error sending comment:', err)
    ...
    // console.log('isTicketLoading in support Ticket ', isTicketLoading)
